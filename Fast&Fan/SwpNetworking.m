...
+ (void)swpPOST:(NSString *)URLString parameters:(nullable NSDictionary *)parameters swpNetworkingSuccess:(SwpNetworkingSuccessHandle)swpNetworkingSuccess swpNetworkingError:(SwpNetworkingErrorHandle)swpNetworkingError {
    ...
    parameters  = [swpNetworking parameterWithUID:parameters];
    // 发起请求
...
+ (void)swpPOSTAddWithFiles:(NSString *)URLString parameters:(NSDictionary *)parameters fileNames:(NSArray *)fileNames fileDatas:(NSArray *)fileDatas  fileTypes:(NSArray*)types swpNetworkingSuccess:(SwpNetworkingSuccessHandle)swpNetworkingSuccess progress:(SwpNetworkingProgress)progress swpNetworkingError:(SwpNetworkingErrorHandle)swpNetworkingError {
    ...
    [swpNetworking.swpSessionManager POST:URLString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        for (int i=0; i<fileDatas.count; i++) {
            if ([types[i] isEqualToString:@"image/png"]) {
                [formData appendPartWithFileData:fileDatas[i] name:@"image" fileName:fileNames[i] mimeType:types[i]];
            }else{
                [formData appendPartWithFileURL:[NSURL fileURLWithPath:fileDatas[i]] name:@"video" fileName:fileNames[i] mimeType:types[i] error:nil];
            }
        }
    } progress:^(NSProgress * _Nonnull uploadProgress) {
...
//        _swpSessionManager.requestSerializer.timeoutInterval 
    }
    return _swpSessionManager;
}

- (NSDictionary *)parameterWithUID:(NSDictionary *)params
{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString *device_id = [defaults objectForKey:@"device_id"];
    //    NSString *os = [defaults objectForKey:@"os"];
    NSString *maufactor = [defaults objectForKey:@"maufactor"];
    //    NSString *v = [defaults objectForKey:@"v"];
    NSString *str = [NSString stringWithFormat:@"%@|%@",device_id,maufactor];
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:params];
    dict[@"device_id"] = str;
    
//    UserStype stype = [MBGChooseStypeTool getUserStype];
//    if (stype == UserStypeA) {
//        dict[@"version_type"] =@"A";
//    }else if (stype == UserStypeB){
//        
//        dict[@"version_type"] =@"B";
//    }else if (stype == UserStypeC){
//        
//        dict[@"version_type"] =@"C";
//    }else if (stype == UserStypeD){
//        
//        dict[@"version_type"] =@"D";
//    }
    
    
    NSString *fb_id = [defaults objectForKey:@"fb_id"];
    if (fb_id ==nil) {
        fb_id = @"";
    }
    dict[@"fb_id"] = fb_id;
...
+ (void)endURLRequest{
     SwpNetworking        *swpNetworking = [SwpNetworking shareInstance];
    [swpNetworking.swpSessionManager.operationQueue cancelAllOperations];
}
